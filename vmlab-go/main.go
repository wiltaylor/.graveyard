...
func cpCommand(args []string) error {
	if len(args) != 4 {
		return errors.New("Expected vmlab cp {toguest|tohost} {vmname} {src} {dest}")
	}
	...
	for _, vm := range vmlabFile.VirtualMachines {
		...
		switch(direction) {
		case "toguest":
				return vmCopyFileToGuest(vm, src, dst)
			case "tohost":
			  return vmCopyFileToHost(vm, src, dst)
			default:
				break
		}
	}
...
func usage() {
	...
	fmt.Println("cp - Copy file into or out of guest")
